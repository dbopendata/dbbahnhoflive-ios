//
//  MBStationSearchViewController.m
//  MeinBahnhof
//
...
@property (nonatomic, strong) NSArray<MBPTSStationFromSearch*>* searchResultArray;
...
@property(nonatomic,strong) MBMarkerMerger* markerMerger;//only used for datastorage
...
@property (nonatomic) NSInteger requestOpnvOngoing;
...
- (void) loadView
{
    ...
    self.markerMerger = [MBMarkerMerger new];
    ...
    [self.searchResultTableView addSubview:self.loadActivity];
...
-(void)configureCurrentType{
    ...
    switch(self.currentType){
        case MBStationSearchTypeTextSearch:
            ...
            [self.searchErrorView setHeaderText:@"Keine Ergebnisse gefunden" bodyText:nil actionButton:nil];
        ...
        case MBStationSearchTypeFavorite:{
            ...
            [self.searchErrorView setHeaderText:nil bodyText:@"Bestimmen Sie Ihre favorisierten Stationen einfach mithilfe des Stern-Icons in der Suche oder Umgebungsanzeige." actionButton:nil];
...
-(void)updateErrorViewForLocation{
    if(self.currentType == MBStationSearchTypeLocation){
        ...
        if(!self.locationManagerAuthorized){
            ...
            [self.searchErrorView setHeaderText:nil bodyText:@"Haltestellen in Ihrer Umgebung werden Ihnen nur angezeigt, wenn Sie den Ortungsdienst aktivieren." actionButton:@"Jetzt erlauben"];
        } else {
            if([MBGPSLocationManager sharedManager].isGettingOneShotLocation){
                //NSLog(@"...getting location");
                [self.searchErrorView setHeaderText:nil bodyText:@"Bitte warten Sie bis Ihre Position bestimmt und die Daten geladen wurden." actionButton:nil];
            } else {
                if(!self.currentUserPosition){
                    //NSLog(@"...no location");
                    [self.searchErrorView setHeaderText:nil bodyText:@"Haltestellen in Ihrer Umgebung werden Ihnen nur angezeigt, wenn Ihre Position bestimmt werden konnte." actionButton:nil];
                } else {
                    if(_requestDBOngoing > 0 || _requestOpnvOngoing > 0){
                        //NSLog(@"...loading data");
                        [self.searchErrorView setHeaderText:nil bodyText:@"Lade Daten..." actionButton:nil];
                    } else {
                        if(self.markerMerger.markers.count == 0){
                            //NSLog(@"...no data");
                            [self.searchErrorView setHeaderText:nil bodyText:@"Keine Ergebnisse gefunden" actionButton:nil];
                        } else {
                            //NSLog(@"...got data");
                            self.searchErrorView.hidden = YES;
                            [self.searchResultTableView reloadData];
...
-(void)searchErrorDidPressActionButton:(MBSearchErrorView *)errorView{
    if(self.currentType == MBStationSearchTypeLocation){
        [self locationAllowButtonTapped:nil];
    }
}
...
-(void)searchResultDelete:(UIButton*)btn{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"Löschen" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"last_three_requests"];
        btn.hidden = YES;
        [self fillPickerWithLastStationsOrMainStations];
        [self.searchResultTableView reloadData];
        self.stationSearchInputField.delegate = self;
    }]];
...
-(void)locationAllowButtonTapped:(id)sender{
...
- (void)showDBStation:(NSDictionary *)stationDict
{
    [self didSelectStation:stationDict];
}
...
-(MBMarker *)mapSelectedMarker{
    return self.markerMerger.markers.firstObject;
}

- (NSArray *)mapNearbyStations {
    return self.markerMerger.markers;
}

-(void)fillPickerWithLastStationsOrMainStations{
    ...
    if (lastRequestedStations.count > 0) {
        ...
        for(NSDictionary* dict in lastRequestedStations){
            ...
            if(pos){
                s.coordinate = CLLocationCoordinate2DMake(pos.firstObject.doubleValue, pos.lastObject.doubleValue);
            }
            s.distanceInKm = dict[@"distanceInKm"];
            s.eva_ids = dict[@"eva_ids"];
            s.hafas_id = dict[@"hafas_id"];
            [list addObject:s];
        }
        self.searchResultArray = list;
        self.searchResultDeleteButton.hidden = NO;
    } else {
        self.searchResultHeaderTitle.text = @"Suchergebnisse";
        self.searchResultArray = @[];//[self mainStations];
        self.searchResultDeleteButton.hidden = YES;
    }
}
...
- (void)viewDidLayoutSubviews
{
    ...
    if(self.currentType == MBStationSearchTypeLocation || self.currentType == MBStationSearchTypeFavorite){
        ...
        [self.loadActivity centerViewInSuperView];
...
{
    NSData *data = [[self sharedUserDefaultsManager] objectForKey:@"last_three_requests"];
    if (data) {
...
- (void) updateLastRequestedStations:(NSDictionary*)station
{
    ...
    for(NSDictionary *dictionary in archivedStations) {
        if ([[dictionary objectForKey:@"id"]longValue]  == [[station objectForKey:@"id"] longValue]) {
            contains = YES;
    ...
    if (!contains) {
        [archivedStations insertObject:station atIndex:0];
        ...
        [archivedStations insertObject:storedDict atIndex:0];
    }
    ...
    NSData *archivedStationsData = [NSKeyedArchiver archivedDataWithRootObject:archivedStations];
    [[self sharedUserDefaultsManager] setObject:archivedStationsData forKey:@"last_three_requests"];
    [[self sharedUserDefaultsManager] synchronize];
}

#define MAX_NUMBER_OF_RESULTS 100

-(void)requestHafasStationsForSearchTerm:(NSString*)searchTerm integratedWithStations:(NSArray<MBPTSStationFromSearch*>*)results{
    NSString *searchTermSafe = searchTerm;
    //searchTermSafe = [searchTermSafe stringByAppendingString:@"?"];//the ? at the end means for hafas that we want more results that might match the search term and not just the one that hafas thinks is correct
    MBStationSearchType lastSearchType = self.currentType;
    BOOL useLocation = self.currentUserPosition != nil && CLLocationCoordinate2DIsValid(self.currentUserPosition.coordinate);
    [[HafasRequestManager sharedManager] requestStopsForSearchterm:searchTermSafe withCoordinate:useLocation coordinate:self.currentUserPosition.coordinate completion:^(NSArray <MBOPNVStation*>*stopLocations) {
        NSLog(@"processing hafas results...");
        ...
        if(stopLocations.count > 0){
            NSMutableArray<MBPTSStationFromSearch*>* mergedResults = [results mutableCopy];
            // NSLog(@"station results %lu: %@",(unsigned long)results.count,results);
            for(MBOPNVStation* station in stopLocations){
                if(mergedResults.count >= MAX_NUMBER_OF_RESULTS){
                    break;
                }
                
                if(station.hasProductsInRangeICEtoS){
                    // NSLog(@"ignore hafas station with trains: %@",dict);
                    continue;
                }
                
                if(station.stationId && station.extId && station.name){
                    MBPTSStationFromSearch* opnvAsStation = [MBPTSStationFromSearch new];
                    //store hafas "id" as "havas_id" and the externalID is "id"
                    //we change the key names here, because "id" is used as a NSNumber for DB-Stations!
                    opnvAsStation.hafas_id = station.stationId;
                    //transform extId string into a real number
                    opnvAsStation.stationId = [NSNumber numberWithLongLong:[station.extId longLongValue]];
                    opnvAsStation.title = station.name;
                    // NSLog(@"add modified hafas station: %@",opnvAsStation);
                    [mergedResults addObject:opnvAsStation];
                }
            }
            self.searchResultArray = mergedResults;
        } else {
...
-(void)setRequestDBOngoing:(NSInteger)requestDBOngoing{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.requestDBOngoing == 0 && self.requestOpnvOngoing == 0){
            [self.loadActivity stopAnimating];
            if(self.mapRequestedSearchUpdate){
...
-(void)setRequestOpnvOngoing:(NSInteger)requestOpnvOngoing{
    _requestOpnvOngoing = requestOpnvOngoing;
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.requestDBOngoing == 0 && self.requestOpnvOngoing == 0){
            [self.loadActivity stopAnimating];
            if(self.mapRequestedSearchUpdate){
...
- (void) updateSearchResults:(NSArray<MBPTSStationFromSearch*>*)stationList searchTerm:(NSString*)searchTerm typeLocation:(BOOL)isLocation
{
    ...
    //we only use the first 25 objects
    if(results.count > 25){
        results = [results subarrayWithRange:NSMakeRange(0, 25)];
    }
    
    if(results.count > MAX_NUMBER_OF_RESULTS){
        results = [results subarrayWithRange:NSMakeRange(0, MAX_NUMBER_OF_RESULTS)];
    } else if(results.count < MAX_NUMBER_OF_RESULTS && !isLocation){
        [self requestHafasStationsForSearchTerm:searchTerm integratedWithStations:results];
        return;
    }
...
//    NSLog(@"updateSearchresults: %d, %@",isLocation, results);
    ...
    if(isLocation){
        //ÖPNV data from hafas api is added separate
        self.markerMerger.trainStations = results;
        [self.searchResultTableView reloadData];
    } else {
        //must be text search
        self.searchResultArray = results;
...
-(void)setSearchResultArray:(NSArray<MBPTSStationFromSearch*> *)searchResultArray{
    self.longPressStation = nil;
    _searchResultArray = [[NSArray alloc] initWithArray:searchResultArray copyItems:YES];
    [self.searchResultTableView reloadData];
    if(self.currentType == MBStationSearchTypeTextSearch){
        if(searchResultArray && searchResultArray.count == 0 && self.stationSearchInputField.text.length >= 2){
            self.searchErrorView.hidden = NO;
        } else {
            self.searchErrorView.hidden = YES;
        }
    }
    if([self tableView:self.searchResultTableView numberOfRowsInSection:0] > 0){
...
- (void) openStation:(NSDictionary*)station andShowWagenstand:(NSDictionary*)wagenstandUserInfo
{
    self.wagenstandUserInfo = wagenstandUserInfo;
    [self didSelectStation:station];
}

- (void) openStationAndShowFacility:(NSDictionary*)station
{
    self.startWithFacilityView = YES;
    [self didSelectStation:station];
}

-(NSArray*)mainStations{
    return @[ @{@"title":@"Berlin Hauptbahnhof", @"id":@(STATION_ID_BERLIN_HBF)},
              @{@"title":@"Frankfurt (Main) Hbf", @"id":@(STATION_ID_FRANKFURT_MAIN_HBF)},
              @{@"title":@"Hamburg Hbf", @"id":@(STATION_ID_HAMBURG_HBF)},
              @{@"title":@"München Hbf", @"id":@(STATION_ID_MUENCHEN_HBF)},
              @{@"title":@"Köln Hbf", @"id":@(STATION_ID_KOELN_HBF)},
              ];
}

#pragma -
#pragma MBStationPickerViewDelegate

-(void)openStation:(NSDictionary *)stationDictionary{
    [self didSelectStation:stationDictionary startWithDepartures:YES];
}

-(MBStation*)stationObjectForDict:(NSDictionary*)stationDictionary{
    NSString *stationName = [stationDictionary objectForKey:@"title"];
    NSNumber *stationId = [stationDictionary objectForKey:@"id"];
    NSArray* evaIds = stationDictionary[@"eva_ids"];
    NSArray* location = stationDictionary[@"location"];
    MBStation* station = [[MBStation alloc] initWithId:stationId name:stationName evaIds:evaIds location:location];
    return station;
}

- (void) didSelectStation:(NSDictionary *)stationDictionary{
    [self didSelectStation:stationDictionary startWithDepartures:NO];
}

- (void) didSelectStation:(NSDictionary *)stationDictionary startWithDepartures:(BOOL)startWithDepartures
{
    if(_didSelectStationRunning){
        NSLog(@"ignored %@, waiting for %@",stationDictionary,_selectedStation);
        return;
    }
    _didSelectStationRunning = YES;
    [[MBTutorialManager singleton] hideTutorials];
    self.selectedStation = stationDictionary;

    [self updateLastRequestedStations:stationDictionary];
    [self.stationSearchInputField resignFirstResponder];

    MBStation* station = [self stationObjectForDict:stationDictionary];
    [MBTrackingManager setCurrentStationForCrashTracking:station.mbId.integerValue];
    
    if(stationDictionary[@"hafas_id"] != nil){
        switch(self.currentType){
            case MBStationSearchTypeTextSearch:
        ...
        timeVC.hafasStation = [MBOPNVStation stationWithId:stationDictionary[@"hafas_id"] name:stationDictionary[@"title"]];
...
- (void) triggerSearchWithQuery:(NSString*)searchTerm andUseLocation:(BOOL)useLocation
{
    ...
    if(useLocation){
        ...
        self.requestOpnvOngoing += 1;
        [[HafasRequestManager sharedManager] requestNearbyStopsForCoordinate:self.currentUserPosition.coordinate filterOutDBStation:YES withCompletion:^(NSArray<MBOPNVStation*> *nearbyStations) {
            if (self.requestOpnvOngoing > 0) {
                if(self.currentType != lastSearchType){
                    NSLog(@"WARNING; SEARCH TYPE CHANGED, IGNORE REQUEST RESULTS");
                } else if (nil != nearbyStations && nearbyStations.count > 0) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self.markerMerger.oepnvStations = nearbyStations;
                        [self.searchResultTableView reloadData];
                    });
                }
                self.requestOpnvOngoing--;
            }
        }];
    } else {
        self.requestDBOngoing += 1;
    ...
    if (notification.userInfo) {
        ...
        if (location && [location.timestamp timeIntervalSinceNow] > -60.0) {
            ...
            if(coordinateChanged || self.triggerSearchAfterLocationUpdate){
                self.triggerSearchAfterLocationUpdate = NO;
                [self.markerMerger removeStations];
                [self.searchResultTableView reloadData];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    switch(self.currentType){
        ...
        case MBStationSearchTypeLocation:{
            NSInteger res = self.markerMerger.markers.count;
            return res;
        }
    }
}
...
-(NSDictionary*)stationDataForCurrentType:(NSIndexPath*)indexPath{
    switch(self.currentType){
        case MBStationSearchTypeTextSearch:
            return self.searchResultArray[indexPath.row].dictRepresentation;
        case MBStationSearchTypeFavorite:{
            NSArray* fav = [[MBFavoriteStationManager client] favoriteStationsList];
            return fav[indexPath.row];
        }
        case MBStationSearchTypeLocation:
            return self.markerMerger.markers[indexPath.row].userData;
    }
    return nil;
}
...
-(void)stationPickerCellDidTapDeparture:(MBStationPickerTableViewCell *)cell{
    NSIndexPath* indexPath = [self.searchResultTableView indexPathForCell:cell];
    NSDictionary* station = [self stationDataForCurrentType:indexPath];
