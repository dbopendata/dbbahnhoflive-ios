//
//  MBStationPickerTableViewCell.m
//  MeinBahnhof
//
...
-(void)setStation:(NSDictionary*)station{
    _station = station;
    self.stationTitleLabel.text = [station objectForKey:@"title"];
    NSNumber* distanceKM = [station objectForKey:@"distanceInKm"];
    ...
    BOOL isOEPNV = station[@"hafas_id"] != nil;
...
-(void)updateDepartures{
    NSArray *eva_ids = [self.station objectForKey:@"eva_ids"];
    NSString *hafas_id = [self.station objectForKey:@"hafas_id"];
    ...
    if (nil != eva_ids && eva_ids.count > 0) {
        ...
        [self.localHafasManager loadDeparturesForStopId:hafas_id timetable:nil withCompletion:^(HafasTimetable *timetable) {
