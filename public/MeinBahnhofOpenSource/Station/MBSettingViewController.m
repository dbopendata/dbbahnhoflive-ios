...
@property(nonatomic,strong) NSMutableArray* favStations;
...
- (void)viewDidLoad {
    ...
    for(NSDictionary* dict in self.favStations){
        if(dict[@"id"] && [self.currentStation.mbId isEqualToNumber:dict[@"id"]]){
            currentStationIsFavorite = YES;
            break;
        }
    }
    if(!currentStationIsFavorite){
        [self.favStations insertObject:@{@"id":self.currentStation.mbId, @"title":self.currentStation.title} atIndex:0];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        ...
        NSDictionary* dict = self.favStations[indexPath.row];
...
-(void)switchChanged:(UISwitch*)sw{
    if(self.selectedCell.section == 0){
        NSDictionary* dict = self.favStations[self.selectedCell.row];
        if(sw.on){
