//
//  MBRootContainerViewController.m
//  MeinBahnhof
//
...
-(void)reloadStationForcedByUser:(BOOL)forcedByUser{
    ...
    _openRequests = 4+1+1;//+1 for the travelcenter and +1 for the EinkaufsbahnhofOverview
    if(requestParking){
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [[MBPTSRequestManager sharedInstance] requestStationData:station.mbId forcedByUser:forcedByUser success:^(MBPTSStationResponse *response) {
            ...
            [self requestTravelCenter:station forcedByUser:forcedByUser del:del];
...
-(void)requestTravelCenter:(MBStation*)station forcedByUser:(BOOL)forcedByUser del:(NSObject*)del{
    //[self changeOpenRequests:-1];
    
    CLLocationCoordinate2D loc = station.positionAsLatLng;
    if(!CLLocationCoordinate2DIsValid(loc)){
        [self changeOpenRequests:-1];
        return;
    }

    [[MBPTSTravelCenterRequestManager sharedInstance] requestTravelcenter:station.mbId location:loc forcedByUser:forcedByUser success:^(MBPTSTravelcenter *response) {
        station.travelCenter = response;
        if([self.rootDelegate respondsToSelector:@selector(didLoadTravelCenter:)] && del == self.rootDelegate){
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.rootDelegate didLoadTravelCenter:YES];
            });
        }
        [self changeOpenRequests:-1];
    } failureBlock:^(NSError *error) {
        if([self.rootDelegate respondsToSelector:@selector(didLoadTravelCenter:)] && del == self.rootDelegate){
            dispatch_async(dispatch_get_main_queue(), ^{
                [self.rootDelegate didLoadTravelCenter:NO];
            });
        }
        [self changeOpenRequests:-1];
    }];
